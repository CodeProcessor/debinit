...
The one-liner at the top clones/updates this repo, then runs `scripts/install-debinit.sh` from your working tree (so you always get the logic from `git`, not a stale pasted script). That script runs the same steps as `just all` using plain bash—no `just` required for install.
